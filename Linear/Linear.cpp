...
double Matrix::Value(size_t i, size_t j) {
	return matrix[i][j];
}
...
Task::Task(std::vector<double>& function, Limitations& limitations, std::vector<bool>& vars_sign, size_t Matrix_size):
	b(limitations.limitations.size()),
	objective_function(vars_in_canonical(function, limitations, vars_sign)),
	A(limitations.limitations.size(), Matrix_size) {}

size_t Task::vars_in_canonical(std::vector<double>& function, Limitations& limitations, std::vector<bool>& vars_sign) {
...
Duality::Duality(std::vector<double>& function, Limitations& limitations, std::vector<bool>& vars_sign)
	: Task(function, limitations, vars_sign, function.size()){
	for (size_t i = 0; i < limitations.limitations.size(); i++) {
		for (size_t j = 0; j < function.size(); j++) {
			if(limitations.limitations[i].second == LT_LE)
				A[i][j] = -limitations.limitations[i].first[j];
			else
				A[i][j] = limitations.limitations[i].first[j];
		}
	}
	...
	A.Transposed();
	A.print();
	std::vector<double> time_arr = b;//временный вектор
	b = objective_function;
	objective_function = time_arr;
	for (size_t i = 0; i < A.get_n(); i++) {
		std::vector<double> time_vect;
		for (size_t j = 0; j < A.get_m(); j++) {
			double num = A.Value(i, j);
			time_vect.push_back(num);
		}
		if (vars_sign[i] == true) {
			limit->add_limitations({ time_vect, LT_LE });
		}
		else {
			limit->add_limitations({ time_vect, LT_EQ });
		}
	}
	for (size_t i = 0; i < limitations.limitations.size(); i++) {
		if (limitations.limitations[i].second == LT_LE)
			var_sign.push_back(true);
		else
			var_sign.push_back(false);
	}
	Check();
}

Linear::Linear(std::vector<double>& function, Limitations& limitations, std::vector<bool>& vars_sign)
	: Task(function, limitations, vars_sign, vars_in_canonical(function, limitations, vars_sign)),
	...
	Check();
	dual_program = new Duality(function, limitations, vars_sign);
}
